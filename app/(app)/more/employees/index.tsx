...
export default () => {
  const r = useRouter(),
    m = q<any>(`SELECT * FROM members WHERE role='EMPLOYEE'`);
  return (
    <Screen title="Employees">
      <Button
        title="+ Invite employee"
        onPress={() => r.push("/(app)/more/employees/invite" as any)}
      />
      {m.map((x) => (
        <Row
          key={x.id}
          onPress={() => r.push(`/(app)/more/employees/${x.id}` as any)}
        >
          ...
          <Text>{x.active ? "Active" : "Disabled"}</Text>
        </Row>
      ))}
    </Screen>
  );
