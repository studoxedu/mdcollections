...
export default () => {
  const r = useRouter(),
    ss = q<any>("SELECT * FROM sales ORDER BY created_at DESC");
  return (
    <Screen title="Sales history">
      <Card>
        <Text style={{ fontWeight: "700" }}>Today · Week · Month · Custom</Text>
        <Text style={{ fontSize: 24, fontWeight: "700", marginTop: 8 }}>
          {money(ss.reduce((a, x) => a + x.total, 0))}
        </Text>
      </Card>
      {ss.length ? (
        ss.map((s) => (
          <Row key={s.id} onPress={() => r.push(`/(app)/sales/${s.id}` as any)}>
